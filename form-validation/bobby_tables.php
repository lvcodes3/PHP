...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // using htmlspecialchars() we can transform any HTML characters in the input into HTML entities
    $raw_name = trim(htmlspecialchars($_POST["name"]));
    // checking if $raw_name already exists in $existing_users
    ...
    // POST Input #2 - character
    // Even though character can only selected by radio button, a user can still change the value in the front-end by inspecting the
    // options and changing the value and submitting the form, so we must validate it still.
...
<form method="post" action="">
    ...
    <p>
        ...
        <input type="text" name="birth_year" value="<?php echo $birth_year;?>" required>
    </p>
    <p>
